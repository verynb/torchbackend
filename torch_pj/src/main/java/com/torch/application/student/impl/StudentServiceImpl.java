...
public class StudentServiceImpl implements StudentService {
  ...
  public List<StudentDetailDto> getAllStudents(Integer pageSize, Integer currentPage) {
    ...
    Page<Student> page = studentRepository.findAll(null, pageable);
  ...
  @Override
  public List<StudentDetailDto> filterStudents(Integer pageSize, Integer currentPage, StudentSearchDto dto) {
    List<StudentDetailDto> list = Lists.newArrayList();
    Pageable pageable = new PageRequest(currentPage, pageSize);
    Page<Student> page = studentRepository.findAll(dto == null ? null : dto.toPredicate(), pageable);
    if (CollectionUtils.isNotEmpty(page.getContent())) {
      page.getContent().forEach(student -> {
        list.add(toDetailDto(student));
      });
  ...
  private StudentDetail toDetail(Student student) {
    ...
    if (!Objects.isNull(student)) {
      ...
      School school = schoolRepository.findOne(student.getSchoolId() == null ? 0 : student.getSchoolId());
      dto.setSchoolName(school == null ? "" : school.getSchoolName());
      User user = userRepository.findOne(student.getSponsorId() == null ? 0 : student.getSponsorId());
