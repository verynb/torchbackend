...
public class ReleaseServiceImpl implements ReleaseService {
    ...
    public void deleteRelease(Long id) {
        ...
        if (release == null && release.getStatus() != 0) {
            throw new TorchException("非草稿批次不能删除");
        }
        List<ReleaseStudent> ress = (List<ReleaseStudent>) releaseStudentRepository.findAll(QReleaseStudent.releaseStudent.batchId.eq(id));
        ress.forEach(re -> {
            Student s = studentRepository.findOne(re.getStudentId() == null ? 0l : re.getStudentId());
            if (s != null) {
                s.setStatus(0);
                studentRepository.save(s);
            }
            releaseStudentRepository.delete(re);
        });
